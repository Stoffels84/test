...
with chauffeur_tab:
    ...
    if grp.empty:
        ...
        # Positionele indices per chauffeur (veilig voor .iloc)
...
with voertuig_tab:
    ...
    if "BusTram_disp" not in df_filtered.columns:
        ...
        if counts.empty:
            ...
            # Positionele indices per voertuigtype
            _idx_by_voertuig = df_filtered.groupby("BusTram_disp", sort=False).indices
...
with locatie_tab:
    ...
    if "Locatie_disp" not in df_filtered.columns:
        ...
        loc_options = sorted(
            [x for x in df_filtered["Locatie_disp"].dropna().unique().tolist() if str(x).strip()]
        )
        ...
        if work.empty:
            ...
            agg = (
                agg.merge(dmin, left_on="Locatie_disp", right_index=True, how="left")
            ...
            if agg.empty:
                ...
                agg_view["Periode"] = "—"
                agg_view.loc[_d_ok, "Periode"] = (
                    agg_view.loc[_d_ok, "Eerste"].dt.strftime("%d-%m-%Y") + " – " +
                    agg_view.loc[_d_ok, "Laatste"].dt.strftime("%d-%m-%Y")
                )

                cols_show = [
                    "Locatie", "Schades", "Unieke_chauffeurs",
                    "Unieke_voertuigen", "Unieke_teamcoaches", "Periode"
                ]
                st.dataframe(
                ...
                # Positionele indices per locatie
                _idx_by_loc = work.groupby("Locatie_disp", sort=False).indices

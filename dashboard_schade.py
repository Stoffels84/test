...
def laad_data_default_or_bytes(file_bytes: bytes | None, *, pad="huishoud.xlsx") -> pd.DataFrame:
    ...
    cat_low_tmp = df["categorie"].astype(str).str.strip().str.lower()
    income_mask = is_income(cat_low_tmp)
    if (
    ...
    # 🔹 Eén keer normaliseren voor hergebruik
    df["cat_norm"] = df["categorie"].astype(str).str.strip().str.lower()
...
# (Geen maandselector meer in de sidebar)

# ============================================================
# 📊 Kernberekeningen (op alle data + geselecteerde maand in tabs)
# ============================================================
is_loon_all = is_income(df_filtered["cat_norm"])

inkomen = df_filtered.loc[is_loon_all, "bedrag"].sum()
uitgaven_vast = df_filtered.loc[(~is_loon_all) & (df_filtered["vast/variabel"] == "Vast"), "bedrag"].sum()
uitgaven_var = df_filtered.loc[(~is_loon_all) & (df_filtered["vast/variabel"] == "Variabel"), "bedrag"].sum()
...
with t_overzicht:
    ...
    # Gauges
    try:
        # 1) Gezondheid over GEFILTERDE periode (hier: alle data)
        scores_all = []
        sparendoel = 0.20
        vaste_max  = 0.50
        ...
        for ym, df_month in df_filtered.groupby(df_filtered["datum"].dt.to_period("M"), sort=True):
            ...
            is_loon = is_income(df_month["cat_norm"])
        ...
        # 2) Uitgaven t.o.v. inkomen
        fig_exp_all = None
        ink_all = df_filtered.loc[is_loon_all, "bedrag"].sum()
        uit_all = df_filtered.loc[~is_loon_all, "bedrag"].sum()
        ...
        with col1:
            st.subheader("🎯 Gemiddelde gezondheid (alle data)")
            if fig_avg:
...
with t_maand:
    ...
    if uitgaven_per_cat.empty:
        ...
        fig_top.add_bar(name="Uitgave (binnen)", x=binnen["cat_title"], y=binnen["bedrag"])
...
with t_budget:
    ...
    if not df_mnd.empty:
        ...
        prev = df[(df["datum"] < maand_start) & (~is_income(df["cat_norm"]))].copy()
        if not prev.empty:
    ...
    if not budget_join.empty:
        ...
        fig_b = go.Figure()
        fig_b.add_bar(name="Budget", y=chart_df["categorie"], x=chart_df["budget"], orientation="h")
        fig_b.add_bar(name="Uitgave (binnen)", y=chart_df.loc[~mask_over, "categorie"], x=chart_df.loc[~mask_over, "uitgave"], orientation="h")
        fig_b.add_bar(name="Uitgave (boven)", y=chart_df.loc[mask_over, "categorie"], x=chart_df.loc[mask_over, "uitgave"], orientation="h", marker_color="crimson")
